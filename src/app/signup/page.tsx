...
import { UserPlus } from 'lucide-react'; // Using UserPlus for differentiation, LogIn is also fine
...
export default function SignupPage() {
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      // Successful sign-up/sign-in will trigger the useEffect above
    } catch (error) {
  ...
  return (
    <div className="container mx-auto flex min-h-[calc(100vh-12rem)] items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-xl">
        ...
        <CardContent>
          <Button onClick={handleGoogleSignUp} disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">
            {loading ? (
